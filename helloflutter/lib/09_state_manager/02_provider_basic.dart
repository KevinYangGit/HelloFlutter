  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Center(
        ...
        child: Consumer<KYCounterViewModel>(
          // 第五步：改变共享数据，会重新创建
          builder: (context, value, child) {
            print('HomePage - Scaffold - Consumer - builder()');
            // 第四步：取值
            // 4.1 直接从参数中取值
...
/*
启动时的打印信息
flutter: HomePage - builder()
flutter: HomePage - Scaffold - Consumer - builder()
flutter: KYTextLessWidget build()
flutter: _KYTextFulWidgetState build()
...
class KYTextLessWidget extends StatelessWidget {
  ...
  Widget build(context) {
    ...
    // 4.2 通过 Provider 取值，必须要传自定义的 Provider
...
class _KYTextFulWidgetState extends State<KYTextFulWidget> {
  ...
  Widget build(context) {
    // 5.2 改变数据时打印
    print('_KYTextFulWidgetState build()');
    // 4.2 通过 Provider 取值，必须要传自定义的 Provider
    int counter = Provider.of<KYCounterViewModel>(context).counter;
    return Text('当前计数：$counter');
  }
